...
CREATE TABLE `company`.`jobs` (
  ...
  `min_salary` DOUBLE(10,2) NULL,
  `max_salary` DOUBLE(10,2) NULL,
  PRIMARY KEY (`job_id`));
...
CREATE TABLE `company`.`counties` (
  ...
  CONSTRAINT `region_id`
    ...
    REFERENCES `company`.`regions` (`region_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
...
CREATE TABLE `company`.`locations` (
  ...
  CONSTRAINT `country_id`
    ...
    REFERENCES `company`.`counties` (`country_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
...
CREATE TABLE `company`.`employee` (
  ...
  CONSTRAINT `job_id`
    FOREIGN KEY (`job_id`)
    REFERENCES `company`.`jobs` (`job_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
...
CREATE TABLE `company`.`dependents` (
  `dependent_id` DECIMAL(10) NOT NULL,
  `first_name` VARCHAR(45) NULL,
  `last_name` VARCHAR(45) NULL,
...
alter table employee modify column salary SMALLINT;
alter table employee add commission varchar(50);


INSERT INTO regions(region_id,region_name) 
VALUES ('1','Europe'), ('2','Americas'), ('3','Asia'), ('4','Middle East and Africa');

INSERT INTO counties (country_id,country_name,region_id) 
...
select concat(employee.first_name," ",employee.last_name) as employee_name, jobs.job_id 
from employee 
inner join jobs on employee.job_id = jobs.job_id 
where jobs.job_title = "Shipping clerk";
select concat(first_name," ",last_name) as employee_id, department_id from employee where department_id = 8;
select department_name from dept order by department_name desc;
select concat(first_name," ",last_name) as employee_name from employee where last_name like "K%";
select concat(first_name," ",last_name) as employee_name,hire_date from employee where year(hire_date) between 1995 and 1997;
select job_title,max_salary from jobs where max_salary <= 5000;
select lower(email) as email from employee;
select concat(first_name," ",last_name) as employee_name from employee where year(hire_date) = 1995;
insert into employee (employee_id,first_name,last_name,email,phone_number, hire_date,job_id,salary,manager_id,department_id) values(207,'Paul','Newton', 'paul.newton@sqltutorial.org','585.123.4567','1987-07-17',2,12000.00,101,11);
select concat(first_name," ",last_name) as employee_name from employee where employee_id = 207;
delete from dept where department_name = "Shipping";
select * from dept where department_name = "shipping";
SET SQL_SAFE_UPDATES = 1;
select concat(first_name," ",last_name) as employee_name from employee where dayname(hire_date) = "Thursday"
